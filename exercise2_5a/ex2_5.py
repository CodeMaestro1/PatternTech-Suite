...
class Dense():
    ...
    def backward(self, dE_dY, learning_rate):
        ...
        dE_dW = dE_dY @ self.input.T
        dE_dX = self.weights.T @  dE_dY
    ...
    def update_weights(self, dE_dW, dE_dB, learning_rate):
        # Correctly updating weights and biases with gradient descent
...
class Sigmoid(Activation):
    def __init__(self):
        ...
        def sigmoid_grad(x):
            act = sigmoid(x)  # Reuse sigmoid to avoid recomputation
            actGrad = act * (1 - act)
            return actGrad
...
def loss_cross_entropy_grad(y_true, y_pred):
    # Fixing the issues by calculating the gradient of cross entropy loss
    epsilon = 1e-15
    y_pred = np.clip(y_pred, epsilon, 1 - epsilon)
...
def train(network, x_train, y_train, epochs, learning_rate, batch_size, verbose = True):
    num_batches = 0
...
train(network, x_train, y_train, epochs=100, learning_rate=0.01, batch_size = 64)
